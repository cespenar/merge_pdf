...
if __name__ == "__main__":
    ...
    layout = [
        ...
        [sg.Button("Merge files", key="button_merge",
                   tooltip="Merge selected files"), sg.Cancel(tooltip="Cancel")],
    ]
    ...
    while True:
        ...
        if event == "button_merge":
            if not values["input"] or not values["output"]:
                sg.popup(
                    "You need to choose both input folder and output file!", title="Warning!")
            else:
